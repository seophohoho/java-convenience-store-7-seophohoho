...
import store.models.ProductDefault;
import store.models.ProductPromotion;
...
public class Output {
    ...
    public static void printProducts(Map<String, ProductDefault> defaultProducts, Map<String, ProductPromotion> promotionProducts){
        for(ProductDefault productDefault: defaultProducts.values()){
            ...
            if(promotionProduct != null){
                System.out.printf(PRINT_PRODUCT_PROMOTION_EXIST, productName, promotionProduct.getPrice(), promotionProduct.getStock(), promotionProduct.getType());
            }

            if(promotionProduct != null && promotionProduct.getStock() == 0){
                System.out.printf(PRINT_PRODUCT_PROMOTION_NOT_EXIST, productName, promotionProduct.getPrice(), promotionProduct.getType());
            }

            if(productDefault.getStock() > 0){
                System.out.printf(PRINT_PRODUCT_DEFAULT_EXIST, productName, productDefault.getPrice(), productDefault.getStock());
            }
